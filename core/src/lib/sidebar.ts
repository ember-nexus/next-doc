...
import { pagePath, commandPath, endpointPath, humanize } from './routes';
...
function buildTree<E extends { id: string }>(
    ...
    const ensureFolder = (segments: string[]): SidebarItem[] => {
        ...
        for (const seg of segments) {
            ...
            if (!items) {
                items = [];
                const group: SidebarGroup = { type: 'group', name: humanize(seg), items };
                parent.push(group);
...
async function apiSection(): Promise<SidebarItem[]> {
    ...
    return [...groups].map(
        ([group, items]): SidebarGroup => ({ type: 'group', name: humanize(group), items }),
    );
}
