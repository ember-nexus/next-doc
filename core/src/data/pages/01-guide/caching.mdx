...
The value of the `ETag` header is calculated as the XXH3 hash from the identifiers and `updated` timestamps of all
involved elements.  
As the `updated` timestamp uses microsecond precision, the resulting `ETag` will be different as long as updates
...
```bash /If-None-Match: .{1,2}[\w\d]+.{1,2}/
curl \
  -H "Authorization: Bearer secret-token:M3WHIDj4q62EY0XiZFMLnv" \
