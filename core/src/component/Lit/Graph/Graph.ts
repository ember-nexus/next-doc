...
export class G6Graph extends LitElement {
    ...
    private get _theme(): Theme {
        ...
        const s: Scheme = this.scheme === 'auto' ? (prefersDark ? 'dark' : 'light') : this.scheme;
        return THEMES[s];
    }
    ...
    private _nodeOptions(): NodeOptions {
        ...
        return {
            ...
            style: {
                ...
                highlight: (d: NodeData) => !!nodeDatum(d).highlight,
                halo: (d: NodeData) => !!nodeDatum(d).highlight,
                haloStroke: (d: NodeData) => typeStyle(nodeDatum(d).type).color,
                haloOpacity: 0.35,
                size: (d: NodeData) => 2 * iconNodeGeometry({
    ...
    private _edgeOptions(): EdgeOptions {
        ...
        return {
            ...
            style: {
                ...
                stroke: t.edgeStroke,
                lineWidth: 2,
                labelText: (d: EdgeData) => edgeDatum(d).name ?? edgeDatum(d).type ?? '',
